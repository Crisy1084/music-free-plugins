...
function $d25ff3d008f9e7e0$var$changeUrlQuery(obj, baseUrl) {
    ...
    Object.keys(newQuery).forEach((key) => {
        if (newQuery[key] !== undefined && newQuery[key] !== "") queryArr.push(
...
async function $d25ff3d008f9e7e0$var$getLyric(musicItem) {
    ...
    let translation;
    if (res.trans) translation = (0, ($parcel$interopDefault($fFvhi$he))).decode((0, ($parcel$interopDefault(
        $fFvhi$cryptojs))).enc.Base64.parse(res.trans).toString((0, ($parcel$interopDefault($fFvhi$cryptojs)))
            .enc.Utf8));
    return {
        rawLrc: (0, ($parcel$interopDefault($fFvhi$he))).decode((0, ($parcel$interopDefault($fFvhi$cryptojs))).enc.Base64
            .parse(res.lyric).toString((0, ($parcel$interopDefault($fFvhi$cryptojs))).enc.Utf8)),
        translation: translation
    };
}
async function $d25ff3d008f9e7e0$var$importMusicSheet(urlLike) {
    let id;
    if (!id) id = (urlLike.match(/https?:\/\/i\.y\.qq\.com\/n2\/m\/share\/details\/taoge\.html\?.*id=([0-9]+)/) ||
        [])[1];
...
async function $d25ff3d008f9e7e0$var$getRecommendSheetTags() {
    const res = (await (0, ($parcel$interopDefault($fFvhi$axios))).get(
...
async function $d25ff3d008f9e7e0$var$getRecommendSheetsByTag(tag, page) {
    const pageSize = 20;
    const rawRes = (await (0, ($parcel$interopDefault($fFvhi$axios))).get(
    ...
    const data = res.list.map((item) => {
        ...
        return {
            ...
            description: item.introduction,
            playCount: item.listennum,
            artist: (_b = (_a = item.creator) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !==
                void 0 ? _b : ""
        };
    });
    return {
...
async function $d25ff3d008f9e7e0$var$getMediaSource(musicItem, quality) {
    let url = "";
    const res1 = (await (0, ($parcel$interopDefault($fFvhi$axios))).get(
        `https://lxmusicapi.onrender.com/url/tx/${musicItem.songmid}/${$d25ff3d008f9e7e0$var$qualityLevels[quality]}`, {
        headers: {
            "X-Request-Key": "share-v2"
        }
    })).data;
    if (!res1.url || res1.msg != "success") {
        const res2 = (await (0, ($parcel$interopDefault($fFvhi$axios))).get(
            `https://lxmusicapi.onrender.com/url/tx/${musicItem.songmid}/${$d25ff3d008f9e7e0$var$qualityLevels[quality]}`, {
            headers: {
                "X-Request-Key": "share-v2"
            }
...
var
    /*search("和你在一起 孙楠", 1, "music").then((res) => {
    console.log(res)
